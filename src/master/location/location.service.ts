...
export class LocationService {
  ...
  async getAllLocation(
    ...
    try {
      ...
      if (search) {
        query
          .andWhere('LOWER(location.location_name) like LOWER(:search)', {
            search: `%${search}%`,
          })
          .orWhere('LOWER(location.factory_name) like LOWER(:search)', {
            search: `%${search}%`,
          });
      }
  ...
  async paginateLocation(
    ...
    try {
      ...
      if (search) {
        page = 1;
        query
          .where('LOWER(location.location_name) like LOWER(:search)', {
            search: `%${search}%`,
          })
          .orWhere('LOWER(location.factory_name) like LOWER(:search)', {
            search: `%${search}%`,
  ...
  private async updateExistingLocation(
    ...
    if (existingLocation.isActive) {
      ...
      const response = await this._locationRepository.save(existingLocation);
  ...
  private async createNewLocation(
    createLocationDto: CreateLocationDto,
  ): Promise<ResponseDto<null>> {
    try {
      ...
      const response = await this._locationRepository.save(newLocation);
