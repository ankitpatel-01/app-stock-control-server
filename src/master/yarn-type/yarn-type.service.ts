...
export class YarnTypeService {
    ...
    async findAllYarnType(
        ...
        try {
            ...
            const query = this._yarnTypeRepository.createQueryBuilder('yarn_type')
                .where("yarn_type.isActive = 1");

            if (search) {
                query.andWhere('LOWER(yarn_type.type_desc) like LOWER(:search)', { search: `%${search}%` });
            }
    ...
    async paginateYarnType(
        ...
        try {
            ...
            const query = this._yarnTypeRepository.createQueryBuilder('yarn_type').where("yarn_type.isActive = 1");

            if (search) {
                page = 1;
                query.where('LOWER(yarn_type.type_desc) like LOWER(:search)', { search: `%${search}%` });
            }
            ...
            if (page == -1) {
                ...
                [data, total] = await query.addOrderBy('yarn_type.id', "DESC").take(limit).skip(skip).getManyAndCount();
            }
    ...
    async createYarnType(createYarnTypeDto: CreateYarnTypeDto): Promise<ResponseDto<null>> {
        try {
            const newYarnType: YarnType = await this._yarnTypeRepository.create({
