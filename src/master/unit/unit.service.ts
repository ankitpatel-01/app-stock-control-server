...
export class UnitService {
    ...
    async getAllUnit(search: string = null): Promise<ResponseDto<Unit[]>> {
        try {
            ...
            const query = this._unitRepository.createQueryBuilder('unit')
                .where("unit.isActive = 1");

            if (search) {
                query.andWhere('LOWER(unit.unit_name) like LOWER(:search)', { search: `%${search}%` })
                    .orWhere('LOWER(unit.unit_desc) like LOWER(:search)', { search: `%${search}%` });
            }
    ...
    async paginateUnit(
        ...
        try {
            ...
            if (search) {
                page = 1;
                query.where('LOWER(unit.unit_desc) like LOWER(:search)', { search: `%${search}%` })
                    .orWhere('LOWER(unit.unit_name) like LOWER(:search)', { search: `%${search}%` });
            }
    ...
    async createUnit(createUnitDto: CreateUnitDto, userId: number): Promise<ResponseDto<null>> {
        ...
        if (existingUnit) {
            if (existingUnit.isActive) {
                ...
                if (respone) {
                    return {
                        message: "new unit create sucessfully",
                        response: null,
                    }
                }
            }
