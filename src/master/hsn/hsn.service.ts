...
export class HsnService {
    ...
    async getAllHsn(search: string = null, gst: boolean = false): Promise<ResponseDto<HSN[]>> {
        try {
            ...
            const query = this._hsnRespository.createQueryBuilder('hsn')
                .where("hsn.isActive = 1");

            if (gst) {
                query.leftJoin('hsn.gst', 'gst').addSelect(['gst.id', 'gst.gst_desc']).where('gst.isActive = 1');
            }

            if (search) {
                query.andWhere('LOWER(hsn.hsn_code) like LOWER(:search)', { search: `%${search}%` })
                    .orWhere('LOWER(hsn.hsn_desc) like LOWER(:search)', { search: `%${search}%` });
            }
    ...
    async paginateHsn(
        ...
        try {
            ...
            if (gst) {
                query.leftJoin('hsn.gst', 'gst').addSelect(['gst.id', 'gst.gst_desc']).andWhere('gst.isActive = 1');
            }
            ...
            if (page == -1) {
                ...
                [data, total] = await query.addOrderBy('hsn.id', "DESC").take(limit).skip(skip).getManyAndCount();
            }
    ...
    async createHsn(createHsnDto: CreateHsnDto): Promise<ResponseDto<null>> {
        try {
            ...
            const newhsn: HSN = await this._hsnRespository.create({
