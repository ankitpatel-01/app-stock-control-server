...
export class LocationController {
    ...
    @Post('create')
    createLocation(@Body() createLocationDto: CreateLocationDto, @GetCurrentUserId() userId: number): Promise<ResponseDto<null>> {
        return this._locationService.createLocation(createLocationDto, userId);
    }

    @Put('update')
    updateLocation(@Body() updateLocationDto: UpdateLocationDto, @GetCurrentUserId() userId: number): Promise<ResponseDto<null>> {
        return this._locationService.updateLocation(updateLocationDto, userId);
    }

    @Delete('remove/:id')
    removeLocation(@Param('id', ParseIntPipe) id: number, @GetCurrentUserId() userId: number): Promise<ResponseDto<null>> {
