...
export class QualityService {
    ...
    async getAllQuality(search: string = null): Promise<ResponseDto<Quality[]>> {
        try {
            const query = this._qualityRespository.createQueryBuilder("q")
                .leftJoinAndSelect("q.type", "type")
                .where("q.isActive = 1")
                .andWhere("type.isActive = 1");

            if (search) {
                query.andWhere("LOWER(q.quality_desc) like LOWER(:search)", { search: `%${search}%` });
            }
    ...
    async paginateQuality(
        ...
        try {
            ...
            if (page == -1) {
                ...
                [data, total] = await query.orderBy("q.id", "DESC").take(limit).skip(skip).getManyAndCount();
            }
    ...
    async createQuality(createQualityDto: CreateQualityDto): Promise<ResponseDto<null>> {
        ...
        try {
            const newQuality: Quality = await this._qualityRespository.create({
