...
export class YarnMasterService {
  ...
  async findAllYarn(search: string = null): Promise<ResponseDto<YarnMaster[]>> {
    try {
      const query = this._yarnMasterRespository
        .createQueryBuilder('yarn')
      ...
      if (search) {
        query
          .andWhere('LOWER(yarn.yarn_desc) like LOWER(:search)', {
            search: `%${search}%`,
          })
          .orWhere('LOWER(yarn.yarn_code) like LOWER(:search)', {
            search: `%${search}%`,
          });
      }
  ...
  async createYarn(createYarnDto: CreateYarnDto): Promise<ResponseDto<null>> {
    try {
      ...
      const newYarn: YarnMaster = this._yarnMasterRespository.create({
