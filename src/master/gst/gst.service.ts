...
export class GstService {
  ...
  async paginateGST(
    ...
    try {
      ...
      if (search) {
        page = 1;
        query.where('LOWER(gst.gst_desc) like LOWER(:search)', {
          search: `%${search}%`,
        });
      }
  ...
  async createGST(createGSTDto: CreateGstDto): Promise<ResponseDto<null>> {
    try {
      const newGST: GST = this._gstRepository.create({
      ...
      const respone = await this._gstRepository.upsert(newGST, ['gst_desc']);
      console.log(respone, 'respnse');
      if (respone) {
        return {
